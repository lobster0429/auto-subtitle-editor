...
export default function Editor({ defaultLang }) {
  ...
  useEffect(() => {
    ...
    const fetchSubtitle = () => {
      const subtitleApi = `${process.env.REACT_APP_API_DOMAIN}/subtitle/${taskId}`;
      const getFilesApi = `${process.env.REACT_APP_API_DOMAIN}/get_subtitle_files/${taskId}`;
      const req = {
        method: 'GET',
        headers: { 
      ...
      fetch(subtitleApi, req)
        .then(res => res.json())
        .then(data => {
          setSubtitleOriginal(data.map((item) => new Sub(item)));
        });

      fetch(getFilesApi, req)
        .then(res => res.json())
        .then(data => {
          if (data.message === 'ok') setFiles(data.result);
        });
    }

    // 如果 localStorage 存有 subtitle 則優先使用
